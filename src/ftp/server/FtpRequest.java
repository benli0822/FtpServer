...
//                rootdir = ((UserInfo) FtpServer.usersInfo.get(i)).workDir;
    ...
    public boolean commandLIST() throws IOException {
        try {
            if (mode == FtpState.FTRANS_PORT) {
                dsocket = new Socket(remoteHost, remotePort);
            } else if (mode == FtpState.FTRANS_PASV) {
                dsocket = ssocket.accept();
            } else {
                return false;
            }
    ...
    public boolean commandTYPE() //响应客户端TYPE指令,切换文件传输的方式，当输入"A"时为ASCII传输，当输入"I"时为二进制编码传输
    {
        if (param.equals("A")) {
